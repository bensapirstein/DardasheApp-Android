...
package dev.bensapirstein.dardashe.ime.text.keyboard
...
import dev.bensapirstein.dardashe.common.Pointer
import dev.bensapirstein.dardashe.common.PointerMap
import dev.bensapirstein.dardashe.common.ViewUtils
import dev.bensapirstein.dardashe.debug.*
import dev.bensapirstein.dardashe.ime.core.*
import dev.bensapirstein.dardashe.ime.keyboard.ComputingEvaluator
import dev.bensapirstein.dardashe.ime.keyboard.DefaultComputingEvaluator
...
import dev.bensapirstein.dardashe.ime.keyboard.KeyboardView
import dev.bensapirstein.dardashe.ime.popup.PopupManager
import dev.bensapirstein.dardashe.ime.text.gestures.GlideTypingGesture
...
import dev.bensapirstein.dardashe.ime.text.gestures.SwipeGesture
import dev.bensapirstein.dardashe.ime.text.key.*
import dev.bensapirstein.dardashe.ime.theme.Theme
import dev.bensapirstein.dardashe.ime.theme.ThemeValue
...
class TextKeyboardView : KeyboardView, SwipeGesture.Listener, GlideTypingGesture.Listener, CoroutineScope {
    ...
    override fun sync() {
        if (isSmartbarKeyboardView) {
            ...
            keyMarginH = ViewUtils.dp2px(prefs.keyboard.keySpacingHorizontal.get()).toInt()
            keyMarginV = ViewUtils.dp2px(prefs.keyboard.keySpacingVertical.get()).toInt()
        }
        fontSizeMultiplier = when (resources.configuration.orientation) {
            Configuration.ORIENTATION_PORTRAIT -> {
                prefs.keyboard.fontSizeMultiplierPortrait.get().toFloat() / 100.0
            }
            Configuration.ORIENTATION_LANDSCAPE -> {
                prefs.keyboard.fontSizeMultiplierLandscape.get().toFloat() / 100.0
            }
            else -> 1.0
        }
